    ...
    if (resp) {
      if (resp.email_verified) {
        if (resp.password == encrypt(data.password)) {
          ...
          let userUpdate = await userModel.update(resp._id, update);
          let categoryCount = await userCategoryModel.getCounts({
            user_id: resp._id,
          });
          if (userUpdate) {
            res.send({
        ...
        sendMail(
          resp.email,
          "One Time Password",
          `<h1>${resp.otp ? resp.otp : update.opt}</h1>`
        );
        res.send({
          status: true,
...
const resetPassword = async (req, res) => {
  ...
  if (!validatorRules.fails()) {
    ...
    if (resp) {
      ...
      let updateResp = await userModel.update({ _id: resp._id }, update);
      if (updateResp) {
        sendMail(
          resp.email,
...
const editPassword = async (req, res) => {
  ...
  try {
    ...
    if (!validatorRules.fails()) {
      ...
      if (resp) {
        if (
          resp.password === encrypt(data.old_password) &&
