const { postVote }   = require('../index');
const { foreach } = require('../../helper/General');

const insert = async (data) => {    
    try
    {
        let row = new postVote();
...
const getRow = async (where, select = []) => {
    try
    {
        let record = await postVote.findOne(where, select);
        return record;
    }
    catch(error)
    {
...
const update = async (id, data) => {
    try
    {
        ...
        let resp = await postVote.updateOne({"_id":id},data);
        if(resp)
        {
...
const getAll = async (where = {}, select = [], joins = [], orderBy = {'created_at':1}, limit = 10) => {
    try {
        let listing = await postVote.find(where, select)
                            .populate(joins)
                            .sort(orderBy)
                            .limit(limit);
...
const getListing = async (req, select = {}, where = {}, joins = []) => {
    try {
        ...
        let listing = postVote.find(where, select, {skip:offset})
                        .populate(joins)
                        .sort(orderBy)
                        .limit(limit)
...
const getCounts = async (where = {}) => {
    try
    {
        let record = await postVote.countDocuments(where);
        return record;
    }
    catch(error)
    {
...
const remove = async (id) => {
    try
    {
        ...
        if(getRecord)
        {
            let record = await postVote.deleteOne({'_id':id});
